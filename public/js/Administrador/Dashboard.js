    ...
    //id = setTimeout($scope.container2, 250);
     if(bool){
      console.log('vacio');  
         ...
         $http.get("Admin/Saa_libs/serie_ventas/"+$scope.lib.CodSucu).then(function(response) {
           ...
           //alert(response.data);
            Highcharts.chart('container',
            {
                ...
                series: [{data:  $scope.graf_ventas[0]}] , 
                title: {text: 'Total de ventas por mes'},
                tooltip: {headerFormat: '<span style="font-size:10px">{series.name}</span><br>',
                pointFormat: '<span style="color:{point.color}">{point.name}</span>: <b>{point.y:.2f} </b> Bs.<br/>'
                }
            });
             //console.log( $scope.graf_ventas[1]);
            });
        }
       ...
       $scope.container2=function (){
$http.get("Admin/Saa_libs/barra_ventas/").then(function(response) {
     $scope.graf_barra=response.data;

$http.get("Admin/Saa_libs/barra_ventas_drilldown/").then(function(response2) {
        $scope.graf_barra_drilldown=response2.data;
  ...
  Highcharts.chart('container2', {
        ...
        subtitle: {
            text: 'Click en la columna de la sucursal que desee evaluar.'
        },
        xAxis: {
            type: 'category'
        },
        yAxis: {
            title: {
                text: 'Total de ingresos hasta la fecha'
            }
        ...
        series: [{
            name: 'Ingresos',
            colorByPoint: true,
            data:$scope.graf_barra
        }],
        drilldown: {
            series: $scope.graf_barra_drilldown
        }
    });
 });
    ...
    if(typeof($scope.lib2.CodSucu)=="undefined"){
      //console.log($scope.lib2); 
       ...
       id = setTimeout($scope.container3, 2050);
        ...
        var dia="";
        if($scope.Fecha_1.getDate()<9)
            dia="0"+$scope.Fecha_1.getDate()+"";
        else
             dia=$scope.Fecha_1.getDate()+"";
        var f=$scope.Fecha_1.getFullYear()+''+mes+''+dia;
        ...
        plotOptions: {
            scatter: {
                ...
                tooltip: {
                    ...
                    pointFormat: '{point.x} Hrs, {point.y} Bs.'
                }
            }
        },
        series: [{
            name: 'Ventas',
            color: 'rgba(223, 83, 84, .4)',
            data: $scope.graf_dispersion[1]
           /* [   [8.45, 51.6], [167.5, 59.0], [159.5, 49.2], [157.0, 63.0], [155.8, 53.6],
